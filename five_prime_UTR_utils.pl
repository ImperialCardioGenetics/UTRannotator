...
sub count_number_ATG{
      ...
      ##TODO: would these all be possible to transcibed? Could link with experimental data
      #
    ...
    sub utr_canonical_splice_sites{
        # return: 2 hash maps
        # 1 - hash map: key - canonical acceptor positions; value - 5'exon start pos
        # 2 - hash map: key - canonical donor positions; value - 3'exon end pos
	    ...
	    my $utr_position = 0;
        my %utr_splice_donor;
        my %utr_splice_acceptor;

        if ($strand == 1){

  	#create a map from chromosome position to UTR position
  	for (my $m=0; $m<$num_exons-1; $m++) {
            $utr_splice_acceptor{$sorted_starts[$m+1] - 1}=$sorted_starts[$m];
            $utr_splice_acceptor{$sroted_starts[$m+1] - 2}=$sorted_starts[$m];
            $utr_splice_donor{$sorted_ends[$m] + 1}=$sorted_ends[$m];
            $utr_splice_donor{$sorted_ends[$m] + 2}=$sorted_ends[$m];
    }
     }

  	    if ($strand == -1){
     for (my $m=$num_exons-2; $m>=0; $m--)
  	{
        $utr_splice_acceptor{$sorted_ends[$m]+1}=$sorted_ends[$m];
        $utr_splice_acceptor{$sorted_ends[$m]+2}=$sorted_ends[$m];
        $utr_splice_donor{$sorted_starts[$m+1]-1}=$sorted_starts[$m];
        $utr_splice_donor{$sorted_starts[$m+1]-2}=$sorted_starts[$m];

  	}}

        return (\%utr_splice_donor, \%utr_splice_acceptor);
    }
